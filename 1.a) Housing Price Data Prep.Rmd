...
```{r}
## Sold Data
nsw_property_05  <- read.csv("C:/Files/Value Australia/All_Projects/Ad Hoc/CL Macroeconomc Work/Property Data/Sold/NSW Raw Sold data - 12 months to December 2005 - inc Geocode_57564_20200624.csv", stringsAsFactors = FALSE) %>% 
  mutate(year = 2005, 
...
nsw_property_06  <- read.csv("C:/Files/Value Australia/All_Projects/Ad Hoc/CL Macroeconomc Work/Property Data/Sold/NSW Raw Sold data - 12 months to December 2006 - inc Geocode_57563_20200624.csv", stringsAsFactors = FALSE) %>% 
  mutate(year = 2006, 
         EventDate = as.Date(EventDate, "%d/%m/%Y"), 
         quarter = as.yearqtr(EventDate),
...
## Rental Data
nsw_rent_05  <- read.csv("C:/Files/Value Australia/All_Projects/Ad Hoc/CL Macroeconomc Work/Property Data/Rental/NSW Raw data (FOR RENT) - 12 months to December 2005 - inc Geocode_57548_20200624.csv", stringsAsFactors = FALSE) %>% 
  mutate(year = 2005, 
         EventDate = as.Date(EventDate, "%d/%m/%Y"), 
         quarter = as.yearqtr(EventDate),
         EventPriceInf = EventPrice * (1+inf_rate)^11) %>% 
...
nsw_rent_06  <- read.csv("C:/Files/Value Australia/All_Projects/Ad Hoc/CL Macroeconomc Work/Property Data/Rental/NSW Raw data (FOR RENT) - 12 months to December 2006 - inc Geocode_57547_20200624.csv", stringsAsFactors = FALSE) %>% 
  mutate(year = 2006, 
         EventDate = as.Date(EventDate, "%d/%m/%Y"), 
         quarter = as.yearqtr(EventDate),
         EventPriceInf = EventPrice * (1+inf_rate)^10) %>% 
...
house_06_median_SA2 <- ddply(house_06, .(SA2_MAIN16), dplyr::summarize, 
                         median_price_inf = median(EventPriceInf),
                         count = length(EventPriceInf) )
...
unit_06_median_SA2 <- ddply(unit_06, .(SA2_MAIN16), dplyr::summarize, 
                         median_price_inf = median(EventPriceInf),
                         count = length(EventPriceInf) )
...
house_06_median_SA3 <- ddply(house_06, .(SA3_CODE16), dplyr::summarize, 
                         median_price_inf = median(EventPriceInf),
                         count = length(EventPriceInf) )
...
unit_06_median_SA3 <- ddply(unit_06, .(SA3_CODE16), dplyr::summarize, 
                         median_price_inf = median(EventPriceInf),
                         count = length(EventPriceInf) )
...
## Rental Prices
# SA2
rental_06_median_SA2 <- ddply(rental_06, .(SA2_MAIN16), dplyr::summarize, 
                         median_rental_inf = median(EventPriceInf),
                         count = length(EventPriceInf) )
...
# SA3
rental_06_median_SA3 <- ddply(rental_06, .(SA3_CODE16), dplyr::summarize, 
                         median_rental_inf = median(EventPriceInf),
                         count = length(EventPriceInf) )
...
## 2006
# Map 2006 data to SA2 Main code
house_06_median_mapped <- house_06_median_SA1 %>% 
...
house_06_median_nd <- anti_join(population_16, house_06_median_mapped, by="SA1_MAIN16") %>% 
  merge(sa1_csv, by="SA1_MAIN16")

# Merge on summarised SA2 data, to impute minimum SA2 values to SA1 areas with no property price data 
house_06_median_nd_wd <- merge(x=house_06_median_nd, y=house_06_median_SA2, by="SA2_MAIN16") %>% 
  dplyr::select(SA1_MAIN16, SA2_MAIN16, median_price_inf)

# Combine with original mobility dataset to obtain full 2006 mobility dataset
house_06_price <- rbind(house_06_median_mapped, house_06_median_nd_wd)

# Merge back on count data
house_06_comb <- merge(x=house_06_price, y=house_06_median_SA1[, c("SA1_MAIN16", "count")], by="SA1_MAIN16", all.x=TRUE)
...
## 2006
# Map 2006 data to SA2 Main code
unit_06_median_mapped <- unit_06_median_SA1 %>% 
  merge(sa1_csv, by="SA1_MAIN16") %>% 
  filter(count > pct) %>% 
  dplyr::select(SA1_MAIN16, SA2_MAIN16, median_price_inf)

# Find SA1 areas without mobility data, subsequently merge on SA1 -> SA2 mapping
unit_06_median_nd <- anti_join(population_16, unit_06_median_mapped, by="SA1_MAIN16") %>% 
  merge(sa1_csv, by="SA1_MAIN16")

# Merge on summarised SA2 data, to impute minimum SA2 values to SA1 areas with no property price data 
unit_06_median_nd_wd <- merge(x=unit_06_median_nd, y=unit_06_median_SA2, by="SA2_MAIN16") %>% 
  dplyr::select(SA1_MAIN16, SA2_MAIN16, median_price_inf)

# Combine with original mobility dataset to obtain full 2006 mobility dataset
unit_06_price <- rbind(unit_06_median_mapped, unit_06_median_nd_wd)

# Merge back on count data
unit_06_comb <- merge(x=unit_06_price, y=unit_06_median_SA1[, c("SA1_MAIN16", "count")], by="SA1_MAIN16", all.x=TRUE)
...
## 2006
# Map 2006 data to SA2 Main code
rental_06_median_mapped <- rental_06_median_SA1 %>% 
  merge(sa1_csv, by="SA1_MAIN16") %>% 
  filter(count > pct) %>% 
  dplyr::select(SA1_MAIN16, SA2_MAIN16, median_rental_inf)

# Find SA1 areas without mobility data, subsequently merge on SA1 -> SA2 mapping
rental_06_median_nd <- anti_join(population_16, rental_06_median_mapped, by="SA1_MAIN16") %>% 
  merge(sa1_csv, by="SA1_MAIN16")

# Merge on summarised SA2 data, to impute minimum SA2 values to SA1 areas with no property rental data 
rental_06_median_nd_wd <- merge(x=rental_06_median_nd, y=rental_06_median_SA2, by="SA2_MAIN16") %>% 
  dplyr::select(SA1_MAIN16, SA2_MAIN16, median_rental_inf)

# Combine with original mobility dataset to obtain full 2006 mobility dataset
rental_06_rental <- rbind(rental_06_median_mapped, rental_06_median_nd_wd)

# Merge back on count data
rental_06_comb <- merge(x=rental_06_rental, y=rental_06_median_SA1[, c("SA1_MAIN16", "count")], by="SA1_MAIN16", all.x=TRUE)
...
property_06 <- merge(house_06_comb, unit_06_comb, by="SA1_MAIN16") %>% 
  merge(rental_06_comb, by="SA1_MAIN16") %>% 
  mutate(property_house_median_price = median_price_inf.x, 
         ...
         property_house_number_sales = IFNA(count.x*2.5,0),
         property_unit_number_sales = IFNA(count.y*2.5,0),
         property_rental_number_bonds = IFNA(count*2.5, 0)) %>% 
