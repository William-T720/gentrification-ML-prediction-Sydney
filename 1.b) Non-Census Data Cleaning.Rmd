...
```{r}
# ABR data
abr_06_raw <- read.csv("C:/Files/Value Australia/Gentrification ML Prediction/Data/Inputs/Non-Census Data/2006_SA1_ABR.csv", stringsAsFactors = FALSE, fileEncoding="UTF-8-BOM")
...
abr_06_wide <- reshape(abr_06_raw, idvar = "SA1_MAIN16", timevar = "Industry_Class", direction = "wide")
...
abr_06_all <- merge(x=population_16, y=abr_06_wide, by="SA1_MAIN16", all.x=TRUE)
...
# 2.3 Export Data
```{r}
write.csv(abr_06, "C:/Files/Value Australia/Gentrification ML Prediction/Data/Inputs/Non-Census Data/CSV_Processed_Inputs/2006_SA1_ABR.csv")
...
sa1_abb_11 <- sa1_abb %>% 
  mutate(airbnb_count = sa1_abb_count_14,
         airbnb_share = sa1_abb_share_14) %>% 
...
## Point of Interest
# Define POI groups
railway <- c("Railway Station")
supermarket <- c("Shopping Centre")
schools <- c("Primary School", "High School", "Preschool", "Combined Primary-High School", "Special School", "Combined Primary-Secondary School")
parks_sport_fields <- c("Park", "Sports Field", "Sports Court", "Golf Course", "Sports Centre")
swimming_place <- c("Swimming Pool", "Beach", "Swimming Pool Facility", "Waterfall")
medical <- c("General Hospital", "Medical Centre", "Emergency Hospital")

# Write sub-group classifications
df_poi <- df_poi %>% mutate(poi_group = ifelse(poitype %in% railway, "railway",
                                               ifelse(poitype %in% supermarket, "shopping centre",
                                                      ifelse(poitype %in% schools, "schools",
                                                             ifelse(poitype %in% parks_sport_fields, "parks sport fields",
                                                                    ifelse(poitype %in% swimming_place, "swimming place",
                                                                           ifelse(poitype %in% medical, "medical", "other")))))))

# Define POI's active as at 2011 and 2016
df_new_poi_11 <- df_poi %>% filter(startdate < "2011-01-01")

df_new_poi_16 <- df_poi %>% filter(startdate < "2016-01-01",
                                   startdate > "2011-01-01")

# Summary test
poi_11_summ <- ddply(df_new_poi_11, .(poi_group), summarise, len = length(poigroup))
poi_16_summ <- ddply(df_new_poi_16, .(poi_group), summarise, len = length(poigroup))
```
...
# Export POI data
write.csv(df_poi_11, "C:/Files/Value Australia/Gentrification ML Prediction/Data/Inputs/Non-Census Data/POI_11.csv")
write.csv(df_poi_16, "C:/Files/Value Australia/Gentrification ML Prediction/Data/Inputs/Non-Census Data/POI_16.csv")
```



########### 5. Prepare SA1 Nodes ###################
# 5.1 Attach centroid of each SA1 area to make it a node
```{r}
# Create SA1 node data by extracting centroid of each SA1 area
sa1_node <- sa1 %>% cbind(st_coordinates(st_centroid(sa1)))

sa1_node$geometry <- NULL

# Export POI data
write.csv(sa1_node, "C:/Files/Value Australia/Gentrification Prediction/Data/Inputs/Non-Census Data/sa1_node.csv")
```
